...
const GameSetup: React.FC<GameSetupProps> = ({ onStart }) => {
  ...
  const isNextDisabled = players.length < 2 || players.length < numberOfTeams;
  let disabledReason = '';
  if (players.length < 2) {
    disabledReason = 'Нужно как минимум 2 игрока.';
  } else if (players.length < numberOfTeams) {
    disabledReason = 'Количество игроков должно быть не меньше количества команд.';
  }
    ...
    <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg w-full max-w-2xl mx-auto">
      ...
      {step === 1 && (
        <>
          ...
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-3 text-gray-700">3. Распределение по командам</h2>
          ...
          <button 
            ...
            className="w-full bg-indigo-600 text-white font-bold py-3 sm:py-4 px-6 text-lg sm:text-xl rounded-lg hover:bg-indigo-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
            Далее
          </button>
          {isNextDisabled && <p className="text-center text-red-500 text-sm mt-2">{disabledReason}</p>}
        </>
      )}
      ...
      {step === 2 && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
            <div>
              <h2 className="text-xl font-semibold mb-3 text-gray-700">1. Время таймера (сек)</h2>
              <div className="flex gap-4 rounded-lg bg-gray-100 p-1">
                {[30, 45, 60].map(time => (
            ...
            <div>
              <h2 className="text-xl font-semibold mb-3 text-gray-700">2. Слов на игрока</h2>
              <input
                  type="number"
          ...
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-3 text-gray-700">3. Сложность слов</h2>
            ...
            <div className="flex flex-col gap-2 rounded-lg bg-gray-100 p-2">
              {Object.values(Difficulty).map((d) => (
                <button 
                  ...
                  className={`w-full p-3 rounded-lg font-medium transition-all duration-200 flex justify-between items-center ${
                ...
                >
                  ...
                  <span className={`text-sm font-medium ${selectedDifficulties.includes(d) ? 'text-indigo-200' : 'text-gray-500'}`}>
                    {wordCounts[d]} слов
                  </span>
                </button>
              ))}
